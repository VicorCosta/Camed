...
import { BaseComponent, AuthenticationService, Empresas } from 'src/app/core';
...
@Component({
  templateUrl: './list-tipos-seguro.component.html'
})

export class ListTiposSeguroComponent extends BaseComponent implements OnInit {
  display = false;
  ...
  @Output() seguro = new EventEmitter<any>();

  constructor(private tiposeguroService: TipoSeguroService,
  ...
  ngOnInit() {
    ...
    this.columnDefinitions = [
      ...
      {
        id: 'grupoAgencia', name: 'Grupo', field: 'grupoAgencia', sortable: true, type: FieldType.string,
  ...
  getData(query) {
    ...
    return this.tiposeguroService.getAll(query);
  }
...
this.angularGrid.gridService.resetGrid();
  ...
  handleEventoConcluido(retorno) {
    if (retorno) {
      this.seguroSelecionado = null;
      this.display = false;
      this.reloadGrid();

      Swal.fire(
        '',
        `Seguro ${retorno.adicionar ? 'adicionado' : 'atualizado'}!`,
        'success'
      );
    }
  }
  ...
  remover(item) {
    Swal.fire({
      title: 'Atenção!',
      text: `Deseja excluir o seguro: ${item.dataContext.nome}?`,
      type: 'warning',
      ...
      preConfirm: () => {
        this.tiposeguroService.deletar(item.dataContext.id)
          .subscribe(response => {
