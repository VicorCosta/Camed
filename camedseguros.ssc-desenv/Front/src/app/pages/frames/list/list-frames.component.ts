...
export class ListFramesComponent extends BaseComponent implements OnInit {
  display = false;
  frameSelecionado: any;
  ...
  getData(query) {
    ...
    return this.framesService.getAll(query);
  }
...
this.angularGrid.gridService.resetGrid();
  ...
  handleEventoConcluido(retorno) {
    if (retorno) {
      this.frameSelecionado = null;
      this.display = false;
      this.reloadGrid();

      Swal.fire(
        '',
        `Frame ${retorno.adicionar ? 'adicionado' : 'atualizado'}!`,
        'success'
      );
    }
  }
  ...
  remover(item) {
    Swal.fire({
      title: 'Atenção!',
      text: `Deseja excluir a frame: ${item.dataContext.nome}?`,
      type: 'warning',
      ...
      preConfirm: () => {
        this.framesService.deletar(item.dataContext.id)
          .subscribe(response => {
