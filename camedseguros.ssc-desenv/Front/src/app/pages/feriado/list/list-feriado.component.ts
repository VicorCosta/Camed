...
export class ListFeriadoComponent extends BaseComponent implements OnInit {
  display = false;
  itemSelecionado: any;
  ...
  ngOnInit() {
    ...
    this.columnDefinitions = [
      ...
      {
        id: 'pais', name: 'País', field: 'pais', sortable: true, type: FieldType.string, filterable: true
      },
      {
        id: 'estado', name: 'Estado', field: 'estado', sortable: true, type: FieldType.string, filterable: true,
      },
      {
      ...
      {
        id: 'data', name: 'Data', field: 'data', sortable: true, type: FieldType.string, filterable: true,
        formatter: (row, cell, value, dataContext) => {
          return moment(value).format('DD/MM/YYYY');
        }
      },
      {
        id: 'descricao', name: 'Descrição', field: 'descricao', sortable: true, type: FieldType.string, filterable: true
      }
    ];
  ...
  getData(query) {
    ...
    return this.feriadoService.getAll(query);
  }
...
this.angularGrid.gridService.resetGrid();
  ...
  handleEventoConcluido(retorno) {
    if (retorno) {
      ...
      Swal.fire(
        '',
        `Feriado ${retorno.adicionar ? 'adicionado' : 'atualizado'}!`,
        'success'
      );
    }
  }
  ...
  remover(item) {
    Swal.fire({
      title: 'Atenção!',
      text: `Deseja excluir o feriado ${item.dataContext.descricao} ?`,
      type: 'warning',
      ...
      preConfirm: () => {
        this.feriadoService.deletar(item.dataContext.id)
          .subscribe(response => {
