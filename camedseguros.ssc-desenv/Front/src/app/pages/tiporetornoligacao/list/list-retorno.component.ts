...
@Component({
  selector: 'app-list-retorno',
  templateUrl: './list-retorno.component.html',
  styleUrls: ['./list-retorno.component.css']
})
export class ListRetornoComponent extends BaseComponent implements OnInit {
  display = false;
  tipoSelecionado: any;
  ...
  @Output() tipo = new EventEmitter<any>();

  constructor(private retornoligacaoService: RetonoligacaoService,
  ...
  getData(query) {
    ...
    return this.retornoligacaoService.getAll(query);
  }
...
this.angularGrid.gridService.resetGrid();
  ...
  handleEventoConcluido(retorno) {
    if (retorno) {
      ...
      Swal.fire(
        '',
        `Tipo ${retorno.adicionar ? 'adicionado' : 'atualizado'}!`,
        'success'
      );
    }
  }
  ...
  remover(item) {
    Swal.fire({
      title: 'Atenção!',
      text: `Deseja excluir o tipo: ${item.dataContext.descricao}?`,
      type: 'warning',
      ...
      preConfirm: () => {
        this.retornoligacaoService.deletar(item.dataContext.id).subscribe(()=>{
