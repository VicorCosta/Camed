...
@Component({
  selector: 'app-list-motivo-cancelamento',
  templateUrl: './list-motivo-cancelamento.component.html',
  styleUrls: ['./list-motivo-cancelamento.component.css']
})
export class ListMotivoCancelamentoComponent extends BaseComponent implements OnInit {
  display = false;
  ...
  @Output() motivo = new EventEmitter<any>();

  constructor(private motivocancelamentoService: MotivoCancelamentoService,
  ...
  getData(query) {
    ...
    return this.motivocancelamentoService.getAll(query);
  }
...
this.angularGrid.gridService.resetGrid();
  ...
  handleEventoConcluido(retorno) {
    if (retorno) {
      ...
      Swal.fire(
        '',
        `Motivo ${retorno.adicionar ? 'adicionado' : 'atualizado'}!`,
        'success'
      );
    }
  }
  ...
  remover(item) {
    Swal.fire({
      title: 'Atenção!',
      text: `Deseja excluir o motivo: ${item.dataContext.descricao}?`,
      type: 'warning',
      ...
      preConfirm: () => {
        this.motivocancelamentoService.deletar(item.dataContext.id).subscribe(()=>{
