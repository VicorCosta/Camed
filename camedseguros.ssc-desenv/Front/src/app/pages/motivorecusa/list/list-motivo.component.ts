...
@Component({
  selector: 'app-list-motivo',
  templateUrl: './list-motivo.component.html',
  styleUrls: ['./list-motivo.component.css']
})
export class ListMotivoComponent extends BaseComponent implements OnInit {
  display = false;
  ...
  constructor(private motivorecusaService: MotivoRecusaService,
  ...
  getData(query) {
    ...
    return this.motivorecusaService.getAll(query);
  }
...
this.angularGrid.gridService.resetGrid();
  ...
  remover(item) {
    Swal.fire({
      ...
      preConfirm: () => {
        this.motivorecusaService.deletar(item.dataContext.id).subscribe(()=>{
