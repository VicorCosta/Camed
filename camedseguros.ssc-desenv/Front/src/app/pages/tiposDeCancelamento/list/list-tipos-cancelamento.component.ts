...
@Component({
  selector: 'app-list-tipos-cancelamento',
  templateUrl: './list-tipos-cancelamento.component.html',
  styleUrls: ['./list-tipos-cancelamento.component.css']
})

export class ListTiposCancelamentoComponent extends BaseComponent implements OnInit {
  display = false;
  itemSelecionado: any;
  ...
  constructor(private cancelamentoService: TiposCancelamentoService,
  ...
  ngOnInit() {
    ...
    this.columnDefinitions = [
      ...
      {
        id: 'nome', name: 'Nome', field: 'descricao', sortable: true, type: FieldType.string,
        filterable: true,
      },
      {
        id: 'tipo', name: 'Grupo', field: 'grupoAgencia', sortable: true, type: FieldType.string,
        filterable: true,
  ...
  getData(query) {
    ...
    return this.cancelamentoService.getAll(query);
  }
...
this.angularGrid.gridService.resetGrid();
  ...
  handleEventoConcluido(retorno) {
    if (retorno) {
      ...
      Swal.fire(
        '',
        `Tipo de cancelamento ${retorno.adicionar ? 'adicionado' : 'atualizado'}!`,
        'success'
      );
    }
  }
  ...
  remover(item) {
    Swal.fire({
      title: 'Atenção!',
      text: `Deseja excluir o tipo de cancelamento: ${item.dataContext.descricao}?`,
      type: 'warning',
      ...
      preConfirm: () => {
        this.cancelamentoService.deletar(item.dataContext.id)
        .subscribe(response => {
      ...
      if (result.value) {
        Swal.fire(
          '',
          'O tipo de agência foi deletado.',
          'success'
        );
      }
    });
