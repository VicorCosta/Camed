...
@Component({
  selector: 'app-list-distribuicao',
  templateUrl: './list-distribuicao.component.html',
  styleUrls: ['./list-distribuicao.component.css']
})

export class ListDistribuicaoComponent extends BaseComponent implements OnInit {
  display = false;
  ...
  constructor(private distribuicaoService: DistribuicaoService,
...
this.angularGrid.gridService.resetGrid();
  ...
  handleEventoConcluido(retorno) {
    if (retorno) {
      ...
      Swal.fire(
        '',
        `Canal de Distribuição ${retorno.adicionar ? 'adicionado' : 'atualizado'}!`,
        'success'
      );
    }
  }
  ...
  remover(item) {
    Swal.fire({
      ...
      if (result.value) {
        Swal.fire(
          '',
          'Canal de Distribuição foi deletado.',
          'success'
        );
      }
    });
