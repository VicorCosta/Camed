...
@Component({
  selector: 'app-list-tipos-agencia',
  templateUrl: './list-tipos-agencia.component.html',
  styleUrls: ['./list-tipos-agencia.component.css']
})

export class ListTiposAgenciaComponent extends BaseComponent implements OnInit {
  display = false;
  itemSelecionado: any;
  ...
  constructor(private agenciaService: TiposAgenciaService,
  ...
  getData(query) {
    ...
    return this.agenciaService.getAll(query);
  }
...
this.angularGrid.gridService.resetGrid();
  ...
  handleEventoConcluido(retorno) {
    if (retorno) {
      ...
      Swal.fire(
        '',
        `Tipo de Agência ${retorno.adicionar ? 'adicionada' : 'atualizada'}!`,
        'success'
      );
    }
  }
  ...
  remover(item) {
    Swal.fire({
      title: 'Atenção!',
      text: `Deseja excluir o tipo de agência: ${item.dataContext.nome}?`,
      type: 'warning',
      ...
      preConfirm: () => {
        this.agenciaService.deletar(item.dataContext.id)
          .subscribe(response => {
